...
{
    public class TypeScriptCompiler_Compile
    {
        public TypeScriptCompiler_Compile()
        {
            file = new Mock<IFile>();
            directory = new Mock<IDirectory>();
            file.SetupGet(f => f.FullPath).Returns("~/test.ts");
            ...
            directory.Setup(d => d.GetFile("~/test.ts")).Returns(file.Object);
        ...
        readonly TypeScriptCompiler compiler;

        [Fact]
        public void Compile_converts_TypeScript_into_JavaScript()
        {
        ...
        [Fact]
        public void Compile_invalid_TypeScript_throws_exception()
        {
            var exception = Assert.Throws<TypeScriptCompileException>(delegate
            {
        ...
        public void Can_Compile_LESS_that_imports_another_LESS_file()
        {
            ...
            compileContext.SourceFilePath = "~/test.ts";
        ...
        public void Using_mixin_from_imported_css_file_throws_exception()
        {
            ...
            Assert.Throws<TypeScriptCompileException>(delegate
            {
                compiler.Compile(
                    "@import \"lib.css\";\nbody{ .mixin; }",
                    compileContext
